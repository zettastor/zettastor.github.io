...
- 分布式块存储软件(下称“DBS”)部署至少需要 **3个节点**，针对不同的使用场景以及客户环境不同，配置参数有所不同，请参考 [高级配置与定制](/configuration)。
...
3. 部署完成之后，可通过浏览器访问 DBS Web 界面，访问地址为 `http://192.168.1.10:8080` 其中 `192.168.1.10` 为修改配置第12步中配置的“Web 界面的 IP 地址”。有关 DBS 的操作与使用，请参考 [用户手册](/manual)。
